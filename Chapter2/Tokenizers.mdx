...
The decode method is useful because models typically output sequences of token IDs (numerical indices), not human-readable text. The decode method:

-Converts token indices back into their corresponding text tokens.
-Merges subword tokens back into complete words (e.g., "▁Hello", "▁world" → "Hello world").
-Handles formatting to produce a fluent, readable sentence.
